...
- (void)showHistory:(UITapGestureRecognizer *)gesture {
    ...
    NSDateFormatter *formatter = P6YObservedFormatter();
    for (NSDictionary *event in [history reverseObjectEnumerator]) {
        ...
        NSString *verb = delta > 0 ? @"gained" : @"lost";
        [lines addObject:[NSString stringWithFormat:@"%@ %ld followers — observed %@", verb, (long)labs(delta), [formatter stringFromDate:date]]];
        if (lines.count == 12) break;
    }
    NSString *events = lines.count ? [lines componentsJoinedByString:@"\n"] : @"No follower-count changes have been observed yet.";
    NSString *message = [NSString stringWithFormat:@"%@\n\nTimes are when P6YTOK observed the count change during a profile visit. TikTok does not provide the exact time each person followed or unfollowed.", events];
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Follower history" message:message preferredStyle:UIAlertControllerStyleAlert];
...
static NSString *P6YProfileUID(id user) {
    for (NSString *key in @[@"uid", @"userID", @"userId", @"secUID", @"secUid", @"uniqueID"]) {
        NSString *value = [P6YProfileGet(user, key) description];
...
static NSNumber *P6YFollowerCount(id user) {
    for (NSString *key in @[@"allFollowersCount", @"followerCount", @"followersCount", @"fansCount"]) {
        NSNumber *value = P6YProfileNumber(P6YProfileGet(user, key));
...
static void P6YRenderFollowerHistory(id adaptor, id user) {
    UIView *view = P6YProfileGet(adaptor, @"view") ?: P6YProfileGet(adaptor, @"headerView");
    ...
    if (![processedUID isEqualToString:uid]) {
        ...
        if (previous && previousVisit > 0) {
            ...
            if (delta > 0) summary = [NSString stringWithFormat:@"%@ followers  •  gained %@ since %@", P6YFollowerCountText(current), P6YFollowerCountText(delta), lastVisit];
            else if (delta < 0) summary = [NSString stringWithFormat:@"%@ followers  •  lost %@ since %@", P6YFollowerCountText(current), P6YFollowerCountText(labs(delta)), lastVisit];
        ...
        if (history.count > 40) [history removeObjectsInRange:NSMakeRange(0, history.count - 40)];
        ...
        objc_setAssociatedObject(adaptor, P6YProfileProcessedUIDKey, uid, OBJC_ASSOCIATION_COPY_NONATOMIC);
    ...
    CGFloat width = MIN(MAX(240, view.bounds.size.width - 24), 390);
...
static void P6YProfileUpdateUI(id self, SEL _cmd, id model) {
    ...
    P6YRenderFollowerHistory(self, model ?: P6YProfileGet(self, @"user"));
}
